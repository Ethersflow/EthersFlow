...
import { getAnalystThesisExcerpt } from '../lib/utils';
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      if (isRequestTooLarge) {
        ...
        if (currentModel !== 'llama-3.3-70b-versatile' && !triedModels.has('llama-3.3-70b-versatile')) {
          console.warn(`[Consensus API fallback] Request too large. Swapping immediately to Llama 3.3 70B.`);
          nextModel = 'llama-3.3-70b-versatile';
        }
      } else if (isDailyQuotaExceeded || isTransientRateLimit) {
        // Multi-tier Intelligent Failover Ladder:
        if (currentModel.includes('nvidia/nemotron-3-ultra-550b') && !triedModels.has('openrouter/nvidia/nemotron-3-super-120b-a12b:free')) {
          console.warn(`[Consensus API failover] Daily quota/limit exhausted on Nemotron Ultra 550B. Failover to Nemotron Super 120B for ${options.slot.name}...`);
          nextModel = 'openrouter/nvidia/nemotron-3-super-120b-a12b:free';
        } else if (currentModel.includes('nvidia/nemotron-3-super-120b') && !triedModels.has('openrouter/nvidia/nemotron-3-ultra-550b-a55b:free')) {
          console.warn(`[Consensus API failover] Daily quota/limit exhausted on Nemotron Super 120B. Failover to Nemotron Ultra 550B for ${options.slot.name}...`);
          nextModel = 'openrouter/nvidia/nemotron-3-ultra-550b-a55b:free';
        } else if ((currentModel.includes('openrouter/') || currentModel.includes('nemotron')) && !triedModels.has('openrouter/google/gemma-4-31b-it:free')) {
          console.warn(`[Consensus API failover] Switching from OpenRouter Nemotron models to OpenRouter Gemma 4 31B for ${options.slot.name}...`);
          nextModel = 'openrouter/google/gemma-4-31b-it:free';
        } else if (!triedModels.has('llama-3.3-70b-versatile')) {
          console.warn(`[Consensus API failover] Fast failover to Groq Llama 3.3 70B for ${options.slot.name}...`);
          nextModel = 'llama-3.3-70b-versatile';
        } else if (!triedModels.has('models/gemini-2.0-flash')) {
          console.warn(`[Consensus API failover] Switching to server-side Gemini 2.0 Flash for ${options.slot.name}...`);
          nextModel = 'models/gemini-2.0-flash';
        } else if (!triedModels.has('llama-3.1-8b-instant')) {
          console.warn(`[Consensus API failover] Switching to Llama 3.1 8B for ${options.slot.name}...`);
          nextModel = 'llama-3.1-8b-instant';
          currentPrompt = trimPromptForLlama(currentPrompt, 3500); // Trim for 8B TPM
        }
      } else {
        // General fallback cascading chain
        if (!triedModels.has('llama-3.3-70b-versatile')) {
          nextModel = 'llama-3.3-70b-versatile';
        } else if (!triedModels.has('models/gemini-2.0-flash')) {
          nextModel = 'models/gemini-2.0-flash';
        } else if (!triedModels.has('llama-3.1-8b-instant')) {
          nextModel = 'llama-3.1-8b-instant';
          currentPrompt = trimPromptForLlama(currentPrompt, 3500);
        }
      }
...
Use bold key-term highlights. Start your output directly with the first section header: "### Thesis & Confidence Quotient".`;
    ...
    try {
      ...
      const primarySynthModel = synthesisModel || 'llama-3.3-70b-versatile';
      
      let synthRaw = "";
      let synthAttempt = 0;
      const maxSynthAttempts = 6;
      let currentSynthModel = primarySynthModel;
      let currentSynthMaxTokens = 4000;

      while (synthAttempt < maxSynthAttempts) {
        try {
          ...
          const maxSynthDuckingAttempts = 2;
          if (isTransientRateLimit && !isDailyQuotaExceeded && synthAttempt <= maxSynthDuckingAttempts) {
            ...
            console.warn(`[Synthesis Rate Limit Ducking] Pausing ${delayMs.toFixed(0)}ms to duck transient rate limit on Synthesis model ${currentSynthModel} (Attempt ${synthAttempt}/${maxSynthDuckingAttempts})...`);
            await new Promise(resolve => setTimeout(resolve, delayMs));
            continue; // Keep retrying synthesis on chosen model!
          }

          // Rapid Failover on Daily Quota or exhausted ducking attempts
          if (currentSynthModel !== 'llama-3.3-70b-versatile' && !errMsg.includes('llama-3.3-70b-versatile')) {
            console.warn(`[Consensus Synthesis Failover] Switching synthesis model from ${currentSynthModel} to llama-3.3-70b-versatile.`);
            currentSynthModel = 'llama-3.3-70b-versatile';
            synthAttempt = 0;
            continue;
          }

          if (currentSynthModel !== 'llama-3.1-8b-instant' && currentSynthModel !== 'models/gemini-2.5-flash') {
            console.warn(`[Consensus Synthesis Failover] Switching synthesis model to llama-3.1-8b-instant.`);
            currentSynthModel = 'llama-3.1-8b-instant';
            synthAttempt = 0;
            synthesisPrompt = trimPromptForLlama(synthesisPrompt, 3500);
            continue;
          }

          if (currentSynthModel !== 'models/gemini-2.5-flash') {
            console.warn("[Consensus Synthesis Failover] Synthesis failed on Llama models, executing final failover to Gemini 2.5 Flash...", errMsg);
            currentSynthModel = 'models/gemini-2.5-flash';
            synthAttempt = 0;
            continue;
          } else {
            throw err;
