...
export function cleanHeadingText(children: React.ReactNode): React.ReactNode {
  ...
  if (typeof children === 'string') {
    return children.replace(/^[#\s\*:]+|[#\s\*:]+$/g, '').replace(/#+/g, '').trim();
  }
  if (Array.isArray(children)) {
    return children.map(child => typeof child === 'string' ? child.replace(/^[#\s\*:]+|[#\s\*:]+$/g, '').replace(/#+/g, '').trim() : child);
  }
  return children;
}
...
export function getAnalystThesisExcerpt(text: string): string {
  ...
  // Normalize header lines so they are easy to match
  let normalized = text;
...
export function parseAnalystReport(text: string): ParsedAnalystReport {
  ...
  // Clean conversational noise or intro phrases
  let cleanText = text.replace(/^\s*(?:here\s+is\s+my\s+analysis|based\s+on\s+the\s+provided\s+grounding|certainly|here\s+is\s+the\s+report|analytical\s+report\s+on\s+.*?)[!:\.,\s]*/gmi, "").trim();
