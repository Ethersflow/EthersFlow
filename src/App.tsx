...
import { cn, cleanHeadingText, stripMarkdown, normalizeAnalystReport, normalizeConsensus, parseAnalystReport } from './lib/utils';
...
import { db, handleFirestoreError, OperationType, getAccessToken, getDriveAccessToken } from './services/firebase';
...
function AgentLibrary({ 
  ...
  toggleShareCustomAgent: (agent: AnalystSlot) => Promise<void>
    ...
    const presetAgents = [
      // venture
      { id: 'vc_generalist', name: 'Venture Capitalist (Generalist)', desc: 'Provides generalist venture investment analysis, funding thesis checks, and general business feedback. Mandated to actively cite and prioritize real-time grounding facts.', category: 'venture', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a seasoned generalist Venture Capitalist. Evaluate the query from a high-level investment perspective. Analyze the general commercial opportunity, strategic viability, and capital efficiency. Keep your analysis high-level, clear, and accessible, avoiding overly narrow sub-specialist jargon unless requested. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'vc_partner', name: 'Venture Partner', desc: 'Specializes in venture fund thesis evaluation, TAM/SAM sizing, unit economics, and competitive moat strength.', category: 'venture', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are an elite Venture Capital Partner evaluating investment opportunities. Assess the market size (TAM/SAM/SOM), competitive landscape, scalability, network effects, and defensive moats. Challenge the founding team\'s underlying growth assumptions. Formulate a strong investment thesis or a clear dissenting view based on market risk, potential exit multiples, and unit economics. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // entrepreneur
      { id: 'startup_generalist', name: 'Startup Advisor (Generalist)', desc: 'General entrepreneurship advice, product-market fit basics, and growth strategy guides. Mandated to actively cite and prioritize real-time grounding facts.', category: 'entrepreneur', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a trusted, generalist Startup Advisor. Evaluate the query to help the builder clarify their business goals, validate initial demand, identify customer value, and formulate a simple execution roadmap. Provide practical, high-level guidance for early-stage startup hurdles. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'pmf_architect', name: 'Product-Market Fit Architect', desc: 'Specializes in entrepreneurship risk, customer pain-point validation, value proposition, and retention loops.', category: 'entrepreneur', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a veteran Product-Market Fit Architect helping early-stage entrepreneurs discover product-market fit. Identify core target customer personas, dissect their pain points, and assess the clarity of the value proposition. Evaluate retention loops, organic viral mechanics, and acquisition costs (CAC vs. LTV). Challenge product design assumptions, suggest specific pilot/MVP metrics to track, and highlight friction points that typically kill early-stage ventures. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // foundations
      { id: 'philanthropy_generalist', name: 'Philanthropy Advisor (Generalist)', desc: 'General charity and impact metrics evaluation, and social foundation support. Mandated to actively cite and prioritize real-time grounding facts.', category: 'foundations', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a generalist Philanthropy Advisor. Evaluate the query or proposal with a focus on general social impact, organizational feasibility, and community alignment. Help guide high-level philanthropic strategies and simple metric tracking. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'grant_auditor', name: 'Grant Auditor', desc: 'Specializes in Theory of Change validation, philanthropic impact metrics, and systemic risk evaluations.', category: 'foundations', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a senior Grant Auditor and Philanthropic Strategist. Evaluate the query or proposal through a rigorous Theory of Change framework. Audit for execution feasibility, long-term financial sustainability, and systemic risk (including unintended downstream dependencies or community friction). Assess the rigor of the proposed success metrics, demanding clear baseline comparisons and robust qualitative/quantitative verification methods. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // b2g
      { id: 'public_sector_generalist', name: 'Public Sector Consultant (Generalist)', desc: 'General public-private partnership advice, compliance basics, and procurement checks. Mandated to actively cite and prioritize real-time grounding facts.', category: 'b2g', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a generalist Public Sector Consultant. Evaluate the query to assess government project feasibility, high-level administrative barriers, public-private alignment, and regulatory compliance paths. Keep your suggestions practical and widely applicable. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'procurement_auditor', name: 'Procurement Auditor', desc: 'Specializes in Government RFP drafts, public sector compliance, and project delivery risk.', category: 'b2g', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are an elite Government Procurement and RFP Compliance Auditor. Analyze the draft, proposal, or procurement query against strict public sector standards, regulatory mandates, and execution criteria. Identify hidden administrative bottlenecks, delivery vulnerabilities, contract compliance loopholes, and resource gaps. Recommend clear risk-allocation clauses, performance benchmarks, and compliance safeguards to ensure seamless public-private execution. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },
      { id: 'policy_analyst', name: 'Policy Analyst', desc: 'Specializes in legislative impact, systemic policy risks, and public interest evaluations.', category: 'b2g', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a veteran Public Policy Analyst. Evaluate the proposed initiative, policy, or legislative draft for socio-economic impact, structural alignment with municipal or federal laws, and stakeholder incentives. Audit for potential administrative friction, political risk, execution costs, and long-term unintended consequences. Challenge policy assumptions with evidence-based alternatives. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // economist
      { id: 'economist_generalist', name: 'General Economist (Generalist)', desc: 'Provides high-level economic insights, supply/demand analyses, and simple macro market commentary. Mandated to actively cite and prioritize real-time grounding facts.', category: 'economist', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a versatile, generalist Economist. Analyze the query using core economic principles (supply and demand, opportunity cost, market structures). Present a balanced, intuitive overview of the economic dynamics at play, suitable for general strategic planning. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'macro_analyst', name: 'Macro Economist', desc: 'Focuses on financial market microstructures, commodities, currency fluctuations, and interest rates.', category: 'economist', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a seasoned Institutional Macro Economist. Analyze the query through the lens of macroeconomic policies, interest rates, inflationary indicators, and global asset pricing (such as currencies, equities, gold, and silver). IMPORTANT: You MUST strictly prioritize and adhere to verified, current real-time grounding facts provided in the analyst notes, actively rejecting stale assumptions or cutoff data. Build a rigorous macroeconomic thesis detailing hedge mechanisms and interest-rate vulnerabilities. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // finance
      { id: 'finance_generalist', name: 'Finance Analyst (Generalist)', desc: 'General financial health auditor, basic cash-flow checks, and standard budgeting feedback. Mandated to actively cite and prioritize real-time grounding facts.', category: 'finance', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a comprehensive Finance Generalist. Analyze the query using fundamental corporate finance principles. Provide high-level advice on financial health, standard capital budgeting, and general risk management without diving deep into overly quantitative debt covenant or DCF mechanics unless prompted. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },

      // legal
      { id: 'legal_generalist', name: 'General Legal Counsel (Generalist)', desc: 'A generalist legal and compliance reviewer for high-level risk and general guidelines. Mandated to actively cite and prioritize real-time grounding facts.', category: 'legal', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a generalist Legal Counsel. Analyze the query for high-level legal risk exposure, common compliance principles, and standard contractual guardrails. Provide structured guidelines to help identify legal questions that require specialized statutory analysis. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'legal_counsel', name: 'Regulatory Counsel', desc: 'Audits legal compliance, risk exposures, jurisdictional structures, and contract vulnerabilities.', category: 'legal', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a corporate Regulatory and Compliance Counsel. Audit the query for regulatory exposure, SEC/CFTC compliance, legal liability, jurisdictional boundaries, contract loopholes, and risk management criteria. Analyze legal precedents, identify disclosure vulnerabilities, and recommend robust compliance frameworks. Be extremely rigorous and detail potential civil, administrative, or statutory exposures. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // academic
      { id: 'academic_generalist', name: 'Academic Generalist (Generalist)', desc: 'A generalist researcher enforcing basic scientific methods, references, and logic rigor. Mandated to actively cite and prioritize real-time grounding facts.', category: 'academic', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a generalist Academic Researcher. Review the query for standard methodological clarity, logical consistency, and reference-backed claims. Help outline standard academic approaches and general research methodologies. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'academic_reviewer', name: 'Academic Reviewer', desc: 'Enforces scientific and academic peer-review rigor, research methodology, and evidence standards.', category: 'academic', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a rigorous Academic Peer Reviewer and Scientific Auditor. Evaluate the query or text through strict empirical methodologies. Audit statistical power, sample selection bias, confounding variables, and causal integrity. Challenge soft science assumptions, identify publication or confirmation biases, and demand robust double-blind controls. Formulate structured critiques of the empirical foundations of the arguments. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // strategic
      { id: 'strategic_generalist', name: 'Strategic Consultant (Generalist)', desc: 'General strategic planning, high-level SWOT analysis, and long-term goal setting. Mandated to actively cite and prioritize real-time grounding facts.', category: 'strategic', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a veteran Strategic Consultant. Evaluate the query to establish a high-level strategic direction, outlining core opportunities, potential risks, and basic operational recommendations. Focus on long-term goal alignment and clear prioritization. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'p1', name: 'Constructive Analyst', desc: 'Constructs the strongest case for a claim.', category: 'strategic', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'Present the strongest case FOR the main claim. Distinguish between evidence-based points and logical inferences. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },
      { id: 'p4', name: 'Ethicist', desc: 'Moral and social implications evaluator.', category: 'strategic', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'Evaluate the query through various ethical frameworks. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },
      { id: 'p7', name: 'Futurist', desc: 'Long-term trend and second-order effect evaluator.', category: 'strategic', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'Analyze the query through the lens of long-term trends and second-order effects. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },
      { id: 'p8', name: 'Out of the Box', desc: 'Specializes in high-entropy, unorthodox game-theoretic viewpoints and Shapley marginal information analysis.', category: 'strategic', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are the "Out of the Box" analyst. Your role is to think completely outside the standard paradigm. Introduce high-entropy, unorthodox, and dissenting perspectives. Utilize advanced game-theoretic principles and look for hidden assumptions or non-linear effects. Challenge conventional consensus and suggest creative, mathematically rigorous alternatives. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // adversarial
      { id: 'adversarial_generalist', name: 'Devil\'s Advocate (Generalist)', desc: 'General adversarial stress testing, identifying obvious loopholes and counter-arguments. Mandated to actively cite and prioritize real-time grounding facts.', category: 'adversarial', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a generalist Devil\'s Advocate. Your sole purpose is to challenge the core assumptions of the query in a constructive yet adversarial manner. Point out obvious flaws, unstated risks, and basic counter-arguments to prevent superficial thinking. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'p2', name: 'Red Team', desc: 'Aggressively finds flaws and contradictions.', category: 'adversarial', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'Your sole mission is to find contradictions, weak evidence, and logical fallacies in the arguments presented by other analysts. Be aggressive but logical. Your role is to prevent groupthink. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },

      // analytical
      { id: 'analytical_generalist', name: 'Analytical Thinker (Generalist)', desc: 'Provides general logical structures, structured reasoning, and rational problem solving. Mandated to actively cite and prioritize real-time grounding facts.', category: 'analytical', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'You are a generalist Analytical Thinker. Approach the query using structured logical thinking, systematic breakdowns, and evidence-based reasoning. Help turn complex problems into clear, sequential components. State your confidence level (HIGH/MEDIUM/LOW) at the start. CRITICAL REAL-TIME DATA DIRECTIVE: You MUST actively leverage, cite, and prioritize the real-time web grounding data (including prices, tickers, and current metrics) provided in the context. Avoid vague generalities or outdated cutoff knowledge; support specialized agents by referencing precise, live-verified facts and citing sources in [Source Name](URL) format.', source: 'preset' },
      { id: 'p3', name: 'The Skeptic', desc: 'Logical auditor for unsupported assumptions.', category: 'analytical', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'Stress-test and challenge claims. Find conceptual flaws, missing evidence, or alternative explanations. Be rigorous. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },
      { id: 'p5', name: 'Empiricist', desc: 'Extracts and verifies empirical claims.', category: 'analytical', model: 'llama-3.3-70b-versatile' as Model, systemPrompt: 'Focus strictly on empirical evidence and statistical significance. State your confidence level (HIGH/MEDIUM/LOW) at the start.', source: 'preset' },
    ...
    return (
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 sm:mb-12 gap-6 border-b border-gray-50 pb-8">
          <div>
            <h2 className="text-3xl sm:text-4xl font-black text-gray-900 tracking-tighter mb-2">Agent Library</h2>
            <p className="text-gray-500 font-bold text-sm sm:text-base">Load specialized reasoning personalities into your collaborative consensus stack.</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            >
              <Plus className="w-4 h-4" /> Create Custom Persona
            </button>
        ...
        {filteredAgents.length === 0 ? (
          <div className="text-center py-20 bg-gray-50/50 rounded-[40px] border border-dashed border-gray-100">
            ...
            <h3 className="text-lg font-black text-gray-900 mb-1">No matching agents found</h3>
            <p className="text-sm text-gray-400 font-bold">Try adjusting your search filters or create a new custom agent.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {agent.source === 'custom' && (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            if (confirm(`Are you sure you want to delete "${agent.name}" from your Persona Library?`)) {
            ...
            >
              ...
              >
                ...
                <h3 className="text-xl sm:text-2xl font-black text-gray-900 mb-2">
                  {editingAgent ? "Edit Custom Persona" : "Create Custom Persona"}
                </h3>
                <p className="text-xs text-gray-400 font-bold mb-6 sm:mb-8 uppercase tracking-wider">
                  {editingAgent ? "Modify saved intelligence module details" : "Architect a new specialized reasoning perspective"}
                </p>
...
export default function App() {
  ...
  const [workspaceNewModel, setWorkspaceNewModel] = useState<Model>('llama-3.3-70b-versatile');
  ...
  const [synthesisEngineModel, setSynthesisEngineModel] = useState<Model>('llama-3.3-70b-versatile');
    ...
    const syncProjectsOnAuthChange = async () => {
      if (!authUser) {
        ...
        try {
          ...
          console.error("Failed to load local projects for guest:", e);
        }
        return;
      }

      // If active user is signed in, load from persistent Firestore database cluster
      try {
        ...
        console.error("Failed loading persistent user projects from Firestore:", e);
        if (e && (String(e).includes('permission') || e.code === 'permission-denied' || e.message?.includes('permission'))) {
          setAuthPermissionError(true);
        }
  ...
  useEffect(() => {
    ...
    // 2. If logged out or currently fetching projects from cloud, prevent update triggers
    if (!authUser || isSyncingWithFirestoreRef.current) {
  ...
  useEffect(() => {
    ...
    if (!authUser || authUser.uid === 'dev-bypass-user') {
    ...
    const q = tenantId
     ...
     const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.warn("Firestore collection subscription failure, attempting fallback retrieval from localStorage:", error);
      if (error && authUser && (String(error).includes('permission') || (error as any).code === 'permission-denied' || (error as any).message?.includes('permission'))) {
        setAuthPermissionError(true);
      }
      try {
        ...
        console.error("Local storage history load failed:", lsErr);
      }
      console.error("Firestore history list subscription error:", error);
    });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.warn("Firestore custom agents subscription error, attempting localStorage fallback:", error);
      if (error && authUser && (String(error).includes('permission') || (error as any).code === 'permission-denied' || (error as any).message?.includes('permission'))) {
        setAuthPermissionError(true);
      }
      try {
        ...
        console.error("Local storage custom agents fallback retrieval failed:", err);
      }
    });
        ...
        isMobile ? "block xl:hidden mb-8 p-6 sm:p-10" : "hidden xl:block p-6 shadow-indigo-50/20"
         ...
         <div className="relative z-10 w-full">
           ...
           <p className={cn(
             ...
             PROPRIETARY COMPRESSION SYNAPSE: SCORE ADJUSTED FOR MODEL DEPENDENCY CLUSTERS AND COGNITIVE DIVERGENCE INDEX.
           </p>
         </div>
      </div>
    );
  ...
  const renderDialogueFlash = () => {
    ...
    if (flashes.length === 0) {
      flashes.push({
        ...
        quote: "Initializing the glass-box multi-model jury cross-examination loop.",
        type: 'thought',
  ...
  if (view === 'auth') {
    return (
      <div className="min-h-screen bg-[#F9F8F6] font-sans text-[#1d1d1f] flex flex-col pt-4">
        ...
        {/* Hero Section */}
        <div className="flex-1 w-full max-w-7xl mx-auto px-6 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-12 lg:py-32 grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-center">
          <div className="max-w-2xl lg:max-w-none text-center lg:text-left py-10">
            <div className="text-[10px] sm:text-xs font-black text-indigo-600 uppercase tracking-[0.3em] md:tracking-[0.4em] lg:tracking-[0.5em] mb-6 sm:mb-10 lg:whitespace-nowrap select-none">{t('hero_tagline')}</div>
            <h1 className="text-5xl sm:text-7xl lg:text-[100px] font-black text-[#1d1d1f] mb-8 sm:mb-12 tracking-tight leading-[0.95] lg:leading-[0.9]">
              EthersFlow <br />
              <span className="text-indigo-600">{t('hero_trustworthy')}</span>
            </h1>
            <p className="text-xl sm:text-2xl lg:text-3xl text-gray-400 font-bold mb-10 sm:mb-16 leading-tight max-w-lg">
              {t('hero_desc_long')} <br />
              <span className="text-indigo-400/60 italic text-lg sm:text-xl font-black">{t('hero_no_hallucinations')}</span>
            </p>
            
            <div className="mb-8 sm:mb-12">
               <details className="group">
                  <summary className="flex items-center justify-center lg:justify-start gap-2 cursor-pointer text-[10px] sm:text-xs font-black text-indigo-400 uppercase tracking-widest hover:text-indigo-600 transition-colors list-none">
                    <AlertTriangle className="w-4 h-4" />
                    {t('problem_title')}
                    <ChevronDown className="w-3 h-3 group-open:rotate-180 transition-transform" />
                  </summary>
                  <div className="mt-4 p-4 sm:p-6 bg-gray-50 rounded-2xl border border-gray-100 text-xs sm:text-sm text-gray-500 font-bold leading-relaxed text-left">
                    {t('problem_desc')}
                  </div>
               </details>
            </div>

            <div className="bg-white rounded-[32px] sm:rounded-[40px] p-6 lg:p-10 shadow-2xl shadow-indigo-100/50 border border-indigo-50">
              {isSignUp && (
                <div className="bg-gradient-to-r from-indigo-50/80 via-purple-50/50 to-indigo-50/80 rounded-2xl p-4 sm:p-5 mb-6 border border-indigo-100 text-left">
                  ...
                  <p className="text-xs text-gray-600 font-bold leading-relaxed">
                    Start testing consensus research queries immediately. No payment info or card authorization needed.
                  </p>
                </div>
              )}
               ...
               <button 
                ...
                className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-100 py-4 rounded-2xl font-black text-gray-900 hover:bg-gray-50 hover:border-indigo-100 transition-all mb-6 shadow-sm cursor-pointer"
              >
              ...
              <form onSubmit={handleEmailAuth} className="space-y-4">
                <input 
                  ...
                  className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 outline-none focus:ring-4 focus:ring-indigo-50 focus:border-indigo-200 transition-all font-bold text-gray-700"
                />
                <input 
                  type="password"
                  required
                ...
                <button 
                  type="submit"
                  className="w-full bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all cursor-pointer"
                >
                  {isSignUp ? "Create Instant Free Account" : t('continue_email')}
                </button>
              </form>
              ...
              <div className="mt-8 text-center space-y-4">
                <button 
                  ...
                  className="text-xs font-black text-indigo-400 hover:text-indigo-600 underline tracking-widest uppercase transition-colors block w-full"
                >
              ...
              <p className="mt-8 text-center text-[10px] text-gray-400 font-medium px-4 leading-relaxed tracking-wide">
                {(() => {
                  const langCode = languageCodeMap[currentLanguage] || 'en';
                  if (langCode === 'fr') {
                    return (
                      <>
                        En continuant, vous reconnaissez la <button type="button" onClick={() => { setView('privacy'); window.scrollTo({ top: 0, behavior: 'instant' }); }} className="text-indigo-500 hover:text-indigo-600 font-bold underline cursor-pointer bg-transparent border-none p-0 inline">Politique de Confidentialité</button> d'EthersFlow et acceptez de recevoir occasionnellement des e-mails promotionnels et des notifications.
                      </>
                    );
                  }
                  if (langCode === 'de') {
                    return (
                      <>
                        Mit dem Fortfahren erklären Sie sich mit der <button type="button" onClick={() => { setView('privacy'); window.scrollTo({ top: 0, behavior: 'instant' }); }} className="text-indigo-500 hover:text-indigo-600 font-bold underline cursor-pointer bg-transparent border-none p-0 inline">Datenschutzerklärung</button> von EthersFlow einverstanden und stimmen dem Erhalt gelegentlicher Werbe-E-Mails und Benachrichtigungen zu.
                      </>
                    );
                  }
                  if (langCode === 'ja') {
                    return (
                      <>
                        続行することにより、EthersFlowの<button type="button" onClick={() => { setView('privacy'); window.scrollTo({ top: 0, behavior: 'instant' }); }} className="text-indigo-500 hover:text-indigo-600 font-bold underline cursor-pointer bg-transparent border-none p-0 inline">プライバシーポリシー</button>に同意し、時折プロモーションメールや通知を受け取ることに同意したものとみなされます。
                      </>
                    );
                  }
                  if (langCode === 'es_es') {
                    return (
                      <>
                        Al continuar, usted acepta la <button type="button" onClick={() => { setView('privacy'); window.scrollTo({ top: 0, behavior: 'instant' }); }} className="text-indigo-500 hover:text-indigo-600 font-bold underline cursor-pointer bg-transparent border-none p-0 inline">Política de Privacidad</button> de EthersFlow y acepta recibir ocasionalmente correos electrónicos promocionales y notificaciones.
                      </>
                    );
                  }
                  return (
                    <>
                      By continuing, you acknowledge EthersFlow's <button type="button" onClick={() => { setView('privacy'); window.scrollTo({ top: 0, behavior: 'instant' }); }} className="text-indigo-500 hover:text-indigo-600 font-bold underline cursor-pointer bg-transparent border-none p-0 inline">Privacy Policy</button> and agree to get occasional promotional emails and notifications.
                    </>
                  );
                })()}
              </p>
        ...
        <div id="pricing" className="w-full bg-[#1d1d1f] py-16 sm:py-32 overflow-hidden relative">
          ...
          <div className="max-w-7xl mx-auto px-6 sm:px-8 md:px-12 lg:px-16 xl:px-24 relative z-10 text-center">
            <div className="text-center mb-10 sm:mb-16">
              <div className="text-[10px] sm:text-[12px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-4 sm:mb-6">{t('plan_deployment') || "Plan Deployment"}</div>
              <h2 className="text-3xl sm:text-4xl lg:text-6xl font-black text-white mb-6 tracking-tighter leading-tight">{t('explore_institutional_plans') || "Explore institutional plans"}</h2>
            </div>
            ...
            <div className="flex flex-col items-center mb-12 sm:mb-20 space-y-8">
              <div className="bg-gray-800/60 p-1 rounded-2xl sm:rounded-3xl flex items-center border border-gray-700/50">
                 ...
                 >
                   {t('team_enterprise') || "Team & Enterprise"}
                 </button>
              </div>
              ...
              {pricingTab === 'individual' ? (
                ...
                >
                  ...
                  <div className="bg-gray-800/40 backdrop-blur-md border border-gray-700 p-6 sm:p-10 rounded-[32px] sm:rounded-[48px] text-left">
                    <h3 className="text-lg sm:text-xl font-black text-white mb-2">{t('free_plan_name') || "Free"}</h3>
                    <p className="text-[10px] sm:text-xs text-gray-400 font-bold mb-4">{t('free_plan_desc') || "Try EthersFlow"}</p>
                    <div className="flex items-baseline gap-1 mb-8">
                       <span className="text-3xl sm:text-4xl font-black text-white">$0</span>
                    </div>
                    <ul className="space-y-3 sm:space-y-4 mb-10">
                      {['3 Intelligence Agents', 'Standard Sandbox Environment', 'Basic Consensus Layer', 'On-Device Zero-Trust Sanitizer Trial'].map((feat, i) => (
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-white text-[#1d1d1f] font-black rounded-2xl hover:bg-gray-100 transition-all text-sm"
                    >
                      {user ? 'Go to Workbench' : 'Try EthersFlow'}
                    </button>
                  </div>
                  ...
                  <div className="bg-white p-8 sm:p-12 rounded-[40px] sm:rounded-[56px] text-left shadow-2xl shadow-indigo-900/40 relative transform sm:scale-105 lg:scale-110">
                    <h3 className="text-xl sm:text-2xl font-black text-[#1d1d1f] mb-2 uppercase tracking-tight">{t('pro_plan_name') || "Pro"}</h3>
                    <p className="text-xs sm:text-sm text-gray-500 font-bold mb-4">{t('pro_plan_desc') || "For everyday productivity"}</p>
                    <div className="flex items-baseline gap-1 mb-8">
                    ...
                    <ul className="space-y-5 mb-12">
                      {['Unlimited Deployed Agents', 'Advanced Discordance Logic', 'Device-Level Zero-Trust Vault', 'Zero-Data-Retention (ZDR) Sovereign Storage'].map((feat, i) => (
                    ...
                    <button 
                      ...
                      className="w-full py-5 bg-indigo-600 text-white font-black rounded-3xl hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all text-sm"
                    >
                      {currentPlan === 'pro' ? 'Current Plan' : 'Try EthersFlow Pro'}
                    </button>
                  </div>

                  {/* Max */}
                  <div className="bg-gray-800/40 backdrop-blur-md border border-gray-700 p-10 rounded-[48px] text-left">
                    <h3 className="text-xl font-black text-white mb-2">{t('max_plan_name') || "Max"}</h3>
                    <p className="text-xs text-gray-400 font-bold mb-4">{t('max_plan_desc', '5-20x more usage than Pro')}</p>
                    <div className="flex items-baseline gap-1 mb-8">
                      <span className="text-4xl font-black text-white">${billingInterval === 'year' ? '80' : '100'}</span>
                      <span className="text-gray-500 font-bold text-xs ml-2">/ month</span>
                    </div>
                    <ul className="space-y-4 mb-10">
                      {['Frontier Model Expert Agent Panel', 'Sovereign Recovery Routing Backup', 'Full Cryptographic Audit Trails', 'Deep Multi-Agent Logic Verification'].map((feat, i) => (
                        <li key={i} className="flex items-center gap-3 text-xs sm:text-sm font-bold text-gray-300">
                          <CheckCircle className="w-4 h-4 text-indigo-400" />
                          {feat}
                        </li>
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-transparent border-2 border-gray-600 text-white font-black rounded-2xl hover:bg-gray-700 transition-all"
                    >
                      Try EthersFlow Max
                    </button>
                  </div>
                ...
                >
                  <div className="bg-gray-800/40 backdrop-blur-md border border-gray-700 p-12 rounded-[56px] text-left">
                    <h3 className="text-2xl font-black text-white mb-2">{t('team_plan_name') || "Team"}</h3>
                    <p className="text-sm text-gray-400 font-bold mb-8 italic">{t('team_plan_desc', 'Collaborative reasoning at scale')}</p>
                    <ul className="space-y-5 mb-12">
                    ...
                    <button 
                      ...
                      className="w-full py-5 bg-white text-[#1d1d1f] font-black rounded-3xl hover:bg-gray-100 transition-all text-sm"
                    >
                      Get Started
                    </button>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 backdrop-blur-md border border-indigo-500/30 p-12 rounded-[56px] text-left">
                    <h3 className="text-2xl font-black text-white mb-2">{t('enterprise_plan_name') || "Enterprise"}</h3>
                    <p className="text-sm text-indigo-300 font-bold mb-8 italic">{t('enterprise_plan_desc', 'Institutional Neural Governance')}</p>
                    <ul className="space-y-5 mb-12">
                    ...
                    <button 
                      ...
                      className="w-full py-5 bg-indigo-600 text-white font-black rounded-3xl hover:bg-indigo-700 shadow-xl shadow-indigo-500/20 transition-all text-sm"
                    >
                      Contact Sales
                    </button>
                  </div>
          ...
          <div className="max-w-3xl mx-auto px-6">
            ...
            <div className="space-y-4">
              {[
                ...
                { 
                  q: "Why does consensus matter?", 
                  a: "Single models, even from premium frontier tiers, are highly susceptible to hallucinations and training biases. Academic research—including seminal studies on multi-agent debate and cognitive diversity in LLMs—has proven that when multiple independent models are forced to cross-examine each other's claims in an adversarial loop, semantic hallucination rates drop by up to 90% and logical coherence is significantly improved. Consensus isn't just a vote; it is a mathematically grounded truth-extraction layer that exposes hidden biases and contradictions before they reach your decision-making pipeline." 
                },
                { 
                  q: "Is my data secure and proprietary?", 
                  a: "Absolutely. EthersFlow is built on a device-level Zero-Trust paradigm. All user queries, files, and conversation histories are tokenized and sanitized locally on-device under our Zero-Trust gateway protocol before any third-party AI model dispatch. We enforce a strict Zero-Data-Retention (ZDR) Sovereign Storage clause, meaning nothing is stored in cloud relays or used to train global frontier models. Institutional enterprise users enjoy isolated, dedicated consensus VPC nodes, TLS 1.3 tunnels, in-memory rate controls, and strict compliance alignment." 
                },
                { 
                  q: "What is your performance SLA?", 
                  a: "EthersFlow provides an active 180-second Multi-Agent Maximum processing SLA. To ensure logical debate never compromises active operations, if any model response or synthesis stalls past 180,000 ms, requests are instantly shifted to secondary Sovereign Recovery routing pipelines. This guarantees responsive, SLA-compliant execution without sacrificing security." 
                },
                { 
                  q: "How do I interpret the Confidence Metric?", 
                  a: "Think of it as a transparent consensus agreement index across our panel of AI experts:\n\n• 90% to 100% (High Consensus): All independent agents fully agree and have successfully verified the empirical evidence without logical friction.\n• 70% to 89% (Stable Consensus): General agreement was reached, but minor analytical nuances or wording differences were noted in the debate.\n• Under 70% (Significant Disagreement / Low Certainty): The agents identified strong logical contradictions, ambiguous source data, or a major evidence gap. These lower scores are often the most valuable outputs, highlighting exact friction points for targeted human audit." 
                }
              ].map((item, i) => (
    ...
    <div className="flex min-h-screen bg-white dark:bg-[#0c0d10] overflow-hidden relative">
        ...
        <header className="hidden lg:flex border-b border-gray-100 dark:border-zinc-800/60 px-8 py-4 items-center justify-between sticky top-0 bg-white/70 dark:bg-[#0c0d10]/70 backdrop-blur-md z-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className={cn(
                "flex items-center gap-3 px-8 py-2.5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-lg active:scale-95",
      ...
      <AnimatePresence>
        {user && currentPlan === 'free' && user.uid !== 'dev-bypass-user' && 
         !['ethersflow.dev@gmail.com', 'ryan.milisits@gmail.com', 'craig@beerwego.com'].includes(user.email?.toLowerCase() || '') && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
      ...
      */}
            ...
            >
              ...
              <div className="w-full md:w-1/3 bg-indigo-50/50 p-6 sm:p-12 border-b md:border-b-0 md:border-r border-gray-100 flex flex-col justify-between shrink-0">
                <div>
                  ...
                  <h3 className="text-2xl sm:text-4xl font-black text-[#1d1d1f] tracking-tighter mb-2 sm:mb-4 leading-tight">Agent Library</h3>
                  <p className="text-gray-500 font-bold leading-relaxed text-xs sm:text-sm">
                    Deploy specialized reasoning perspectives from our validated persona gallery or architect your own.
                  </p>
                </div>
              ...
              <div className="flex-1 p-6 sm:p-12 overflow-y-auto md:max-h-[92vh] custom-scrollbar">
                {customAgents && customAgents.length > 0 && (
                  <div className="mb-8">
                    <div className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-4">My Persona Archive</div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mb-4 sm:mb-6">Agent Persona Gallery</div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-8 w-full">
        ...
        <div className="mb-12 text-center max-w-2xl mx-auto">
              <h2 className="text-3xl font-black text-[#1d1d1f] mb-3 tracking-tighter leading-tight">{t('onboarding_title') || "Collective Intelligence Deployment"}</h2>
              <p className="text-gray-500 font-bold tracking-tight">{t('onboarding_desc', 'Configure your agents below and issue a directive to begin multi-model analysis collective.')}</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:flex lg:flex-row lg:flex-wrap gap-4 items-stretch">
              {slots.map((slot) => (
                <motion.div 
                  ...
                  key={slot.id}
                  className={cn(
                    "p-5 rounded-[28px] border transition-all relative group min-w-0 lg:w-72 lg:flex-shrink-0",
                    slot.active 
                ...
                >
                  ...
                  <div className="mb-4" onClick={(e) => e.stopPropagation()}>
                    <textarea 
                      ...
                      placeholder="Enter detailed instructions or a custom persona description for this agent..."
                      className="w-full text-xs font-semibold text-gray-500 bg-gray-50/70 border border-gray-100 hover:border-gray-200 focus:border-indigo-300 focus:bg-white focus:ring-4 focus:ring-indigo-50/50 rounded-2xl px-3.5 py-3 outline-none transition-all resize-y min-h-[140px] max-h-[220px] custom-scrollbar placeholder:text-gray-300"
                    />
                  </div>
                </div>
                ...
                >
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2 block">
                    {t('intelligence_architecture')}
                  </label>
                  <div className="relative">
                    <select 
                      value={slot.model}
                      ...
                      className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-2.5 text-xs font-bold text-gray-700 outline-none focus:ring-2 focus:ring-indigo-100 transition-all appearance-none cursor-pointer"
                    >
              ...
              <div className="bg-gradient-to-br from-indigo-950 via-slate-905 to-slate-950 text-white rounded-3xl p-5 sm:p-8 shadow-2xl relative overflow-hidden border border-indigo-500/15">
                ...
                <div className="relative z-10 flex flex-wrap items-center justify-between gap-4 mb-6 pb-6 border-b border-white/5">
                  <div className="space-y-1.5 max-w-xl">
                    <div className="inline-flex items-center gap-2 px-2.5 py-0.5 bg-indigo-500/15 border border-indigo-500/20 rounded-full">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-[0.2em] text-indigo-200">
                        GLASS-BOX ADVERSARIAL CONSENSUS
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2.5 rounded-2xl backdrop-blur-sm">
                    ...
                    <div>
                      <div className="text-[8px] font-black text-indigo-300 uppercase tracking-widest leading-none mb-0.5">PROGRESS</div>
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 sm:mb-10">
                            <div>
                              ...
                              <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-indigo-950">
                                {t('consensus_narrative') || "Consensus Narrative"}
                              </h2>
                            </div>
                            <div className="flex items-center gap-2">
                              ...
                              <div className="text-lg text-gray-800 leading-relaxed font-medium whitespace-pre-wrap markdown-content">
                                <ReactMarkdown
                                  ...
                                  components={{
                                    ...
                                    h1: ({ ...props }) => (
                                      <h4 className="text-sm font-black uppercase tracking-wider text-slate-900 mt-6 mb-2">
                                    ...
                                    h3: ({ ...props }) => (
                                      <h4 className="text-xs sm:text-sm font-black uppercase tracking-wider text-slate-900 mt-6 mb-2">
                                        {cleanHeadingText(props.children)}
                                      </h4>
                                    ),
                                    h4: ({ ...props }) => {
                                      ...
                                      if (isThesis) {
                                        return (
                                          <div className="mt-6 mb-3 flex items-center gap-2 bg-indigo-50/70 border border-indigo-100/50 px-3 py-1.5 rounded-xl w-fit">
                                            ...
                                            <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-indigo-950">
                                      ...
                                      return (
                                        <h4 className="text-xs sm:text-sm font-black text-slate-800 mt-5 mb-2">
                                          {cleanHeadingText(props.children)}
                                        </h4>
                                      );
                                    },
                                    ...
                                    h6: ({ ...props }) => (
                                      <h6 className="text-[11px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider mt-3 mb-1">
                                        {cleanHeadingText(props.children)}
                                      </h6>
                                    ),
                                    strong: ({ ...props }) => (
                                    ...
                                    if (isThesis) {
                                      return (
                                        <div className="mt-6 mb-3 flex items-center gap-2 bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-100/50 dark:border-indigo-900/50 px-3 py-1.5 rounded-xl w-fit">
                                          <span className="w-2 h-2 bg-indigo-600 rounded-full animate-pulse" />
                                          <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-indigo-950 dark:text-indigo-300">
                                            Thesis & Confidence Quotient
                                          </span>
                                        </div>
                                      );
